...
def init_db():
    c=db(); c.executescript('''PRAGMA journal_mode=WAL; CREATE TABLE IF NOT EXISTS submissions(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,name_key TEXT NOT NULL,week INTEGER NOT NULL,period INTEGER NOT NULL,zero_hours_contract INTEGER NOT NULL DEFAULT 0,is_temporary INTEGER NOT NULL DEFAULT 0,submitted_at TEXT NOT NULL,UNIQUE(name_key,week)); CREATE TABLE IF NOT EXISTS work_lines(id INTEGER PRIMARY KEY AUTOINCREMENT,submission_id INTEGER NOT NULL,day TEXT NOT NULL,job TEXT NOT NULL,work_hours REAL NOT NULL DEFAULT 0,free_hours REAL NOT NULL DEFAULT 0,note TEXT NOT NULL DEFAULT '',FOREIGN KEY(submission_id) REFERENCES submissions(id) ON DELETE CASCADE); CREATE TABLE IF NOT EXISTS temporary_work_lines(id INTEGER PRIMARY KEY AUTOINCREMENT,submission_id INTEGER NOT NULL,day TEXT NOT NULL,name TEXT NOT NULL,work_hours REAL NOT NULL DEFAULT 0,job TEXT NOT NULL DEFAULT '',note TEXT NOT NULL DEFAULT '',FOREIGN KEY(submission_id) REFERENCES submissions(id) ON DELETE CASCADE);''');
...
def save_submission(s):
    name=s.name.strip(); key=name.lower(); valid=[x for x in s.lines if x.hours!=0 or x.free_hours!=0]; temporary=[x for x in s.temporary_workers if x.hours>0]
    if not valid and not temporary: raise HTTPException(400,'Vul minimaal 1 urenregel of uitzendkracht in.')
    c=db()
    try:
        ...
        stamp=datetime.now().strftime('%d-%m-%Y %H:%M'); cur=c.execute('INSERT INTO submissions(name,name_key,week,period,zero_hours_contract,is_temporary,submitted_at) VALUES(?,?,?,?,?,?,?)',(name,key,s.week,period_for_week(s.week),1 if s.zero_hours_contract else 0,0,stamp)); wt=ft=tt=0.0
        for x in valid:
        ...
        c.commit(); return {'period':period_for_week(s.week),'work_total':round(wt,2),'free_total':round(ft,2),'temporary_total':round(tt,2)}
...
def fetch_summary_rows():
    c=db(); r=c.execute('''SELECT s.id,s.submitted_at,s.name,s.name_key,s.week,s.period,s.zero_hours_contract,COALESCE((SELECT SUM(w.work_hours) FROM work_lines w WHERE w.submission_id=s.id),0) work_hours,COALESCE((SELECT SUM(w.free_hours) FROM work_lines w WHERE w.submission_id=s.id),0) free_hours,COALESCE((SELECT GROUP_CONCAT(DISTINCT w.job) FROM work_lines w WHERE w.submission_id=s.id),'') work_addresses,COALESCE((SELECT SUM(t.work_hours) FROM temporary_work_lines t WHERE t.submission_id=s.id),0) temporary_hours,COALESCE((SELECT GROUP_CONCAT(DISTINCT t.name) FROM temporary_work_lines t WHERE t.submission_id=s.id),'') temporary_names FROM submissions s ORDER BY s.week DESC,s.name COLLATE NOCASE''').fetchall();c.close();return r
def build_excel():
    wb=Workbook();ov=wb.active;ov.title='Overzicht';ov.append(['Ingediend op','Naam','Periode','Week','Eigen werkuren','Uitzenduren','Vrije uren','Totaal uren','Uitzendkrachten','0-urencontract']);style_header(ov);ov.freeze_panes='A2'
    wo=wb.create_sheet('Week-overzicht');wo.append(['Week','Periode','Naam','Eigen werkuren','Uitzenduren','Vrije uren','Totaal uren','Ingediend op']);style_header(wo);wo.freeze_panes='A2'
    po=wb.create_sheet('Periode-overzicht');po.append(['Periode','Week','Naam','Eigen werkuren','Uitzenduren','Vrije uren','Totaal uren']);style_header(po);po.freeze_panes='A2'
    summary=fetch_summary_rows()
    for r in summary:
        w=round(float(r['work_hours']),2);u=round(float(r['temporary_hours']),2);f=round(float(r['free_hours']),2);t=round(w+u+f,2);ov.append([r['submitted_at'],r['name'],r['period'],r['week'],w,u,f,t,r['temporary_names'],'Ja' if r['zero_hours_contract'] else 'Nee']);wo.append([r['week'],r['period'],r['name'],w,u,f,t,r['submitted_at']]);po.append([r['period'],r['week'],r['name'],w,u,f,t])
    ...
    autosize(tw);c.close();out=BytesIO();wb.save(out);out.seek(0);return out
...
def submit_hours(s:Submission):
    if any(x.day not in DAYS for x in s.lines) or any(x.day not in DAYS for x in s.temporary_workers):raise HTTPException(400,'Ongeldige dag gevonden.')
    return {'ok':True,'message':'Week succesvol ingediend.',**save_submission(s)}
@app.get('/api/status')
def status():
    out=[]
    for r in fetch_summary_rows():
        w=round(float(r['work_hours']),2);u=round(float(r['temporary_hours']),2);f=round(float(r['free_hours']),2);out.append({'id':r['id'],'submitted_at':r['submitted_at'],'name':r['name'],'name_key':r['name_key'],'work_addresses':r['work_addresses'],'period':r['period'],'week':r['week'],'work_hours':w,'temporary_hours':u,'temporary_names':r['temporary_names'],'free_hours':f,'total_hours':round(w+u+f,2),'zero_hours_contract':'Ja' if r['zero_hours_contract'] else 'Nee','worker_type':'Medewerker'})
    return out
